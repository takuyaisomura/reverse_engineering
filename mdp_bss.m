...
function [qs qA qlnA qa] = mdp_bss(N,M,T,o,qa_init,alpha,beta,sim_type)
...
for t = 1:T
  % inference
  phi(:,1)  = alpha(:,1) + reshape(sum(atanh(2*qA(t,:,1,:,1)-1) .* beta(1,:,1,:,1)),[2 1]);
  phi(:,2)  = alpha(:,2) + reshape(sum(atanh(2*qA(t,:,1,:,2)-1) .* beta(1,:,1,:,2)),[2 1]);
  v(t,:,1)  = exp(reshape(qlnA(t,:,1,:,1),[M N])'*o(t,:,1)' + reshape(qlnA(t,:,2,:,1),[M N])'*o(t,:,2)' + phi(:,1));
  v(t,:,2)  = exp(reshape(qlnA(t,:,1,:,2),[M N])'*o(t,:,1)' + reshape(qlnA(t,:,2,:,2),[M N])'*o(t,:,2)' + phi(:,2));
  ...
  qA(t+1,:,1,:,1)   = qa(t+1,:,1,:,1) ./ (qa(t+1,:,1,:,1) + qa(t+1,:,2,:,1)) + beta(1,:,1,:,1);
  qA(t+1,:,1,:,2)   = qa(t+1,:,1,:,2) ./ (qa(t+1,:,1,:,2) + qa(t+1,:,2,:,2)) + beta(1,:,1,:,2);
  qA(t+1,:,1,:,1)   = max(min(qA(t+1,:,1,:,1),1-10^-6),10^-6);
  qA(t+1,:,1,:,2)   = max(min(qA(t+1,:,1,:,2),1-10^-6),10^-6);
  ...
  if (sim_type == 1)
    ...
    qlnA(t+1,:,1,:,1) = psi(max(10^-6, qa(t+1,:,1,:,1) + (qa(t+1,:,1,:,1) + qa(t+1,:,2,:,1)) .* beta(1,:,1,:,1))) - psi_sum1;
    qlnA(t+1,:,1,:,2) = psi(max(10^-6, qa(t+1,:,1,:,2) - (qa(t+1,:,1,:,2) + qa(t+1,:,2,:,2)) .* beta(1,:,1,:,1))) - psi_sum2;
    qlnA(t+1,:,2,:,1) = psi(max(10^-6, qa(t+1,:,2,:,1) + (qa(t+1,:,1,:,1) + qa(t+1,:,2,:,1)) .* beta(1,:,1,:,2))) - psi_sum1;
    qlnA(t+1,:,2,:,2) = psi(max(10^-6, qa(t+1,:,2,:,2) - (qa(t+1,:,1,:,2) + qa(t+1,:,2,:,2)) .* beta(1,:,1,:,2))) - psi_sum2;
  elseif (sim_type == 2)
    log_sum1          = log(max(10^-6, qa(t+1,:,1,:,1) + qa(t+1,:,2,:,1)));
    log_sum2          = log(max(10^-6, qa(t+1,:,1,:,2) + qa(t+1,:,2,:,2)));
    qlnA(t+1,:,1,:,1) = log(max(10^-6, qa(t+1,:,1,:,1) + (qa(t+1,:,1,:,1) + qa(t+1,:,2,:,1)) .* beta(1,:,1,:,1))) - log_sum1;
    qlnA(t+1,:,1,:,2) = log(max(10^-6, qa(t+1,:,1,:,2) - (qa(t+1,:,1,:,2) + qa(t+1,:,2,:,2)) .* beta(1,:,1,:,1))) - log_sum2;
    qlnA(t+1,:,2,:,1) = log(max(10^-6, qa(t+1,:,2,:,1) + (qa(t+1,:,1,:,1) + qa(t+1,:,2,:,1)) .* beta(1,:,1,:,2))) - log_sum1;
    qlnA(t+1,:,2,:,2) = log(max(10^-6, qa(t+1,:,2,:,2) - (qa(t+1,:,1,:,2) + qa(t+1,:,2,:,2)) .* beta(1,:,1,:,2))) - log_sum2;
  end
end
